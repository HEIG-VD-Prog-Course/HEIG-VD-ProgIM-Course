package ch.comem.serie5_Suite;

public class ProgrammeAfficheEchiquier {
    private static final char BLANC = 'O';
    private static final char NOIR = '#';

    public static void main(String[] args) {
        char[][] echiquier = rendEchiquier();
        afficheEchiquier(echiquier,true);
        System.out.println();
        afficheEchiquierWaouh(echiquier,true);
    }

    public static char[][] rendEchiquier() {
        return new char[][]{
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
                {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '},
        };
    }

    public static void afficheEchiquier(char[][] echiquier, boolean avecNomCases) {
        int cmpt = 1;
        if (avecNomCases) {
            afficheNomsCols();
        }
        for (int lgn = 8; lgn >= 1; lgn--) {
            if (avecNomCases) {
                System.out.print(lgn + "  ");
            }
            for (char col = 'A'; col <= 'H'; col++) {
                int iLgn = rendIndexLgn(lgn);
                int iCol = rendIndexCol(col);
                if (echiquier[iLgn][iCol] != ' ') {
                    System.out.print(echiquier[iLgn][iCol]);
                } else {
                    if (cmpt % 2 == 0) {
                        System.out.print(NOIR);
                    } else {
                        System.out.print(BLANC);
                    }
                }
                System.out.print("  ");
                cmpt++;
            }
            if (avecNomCases) {
                System.out.print(lgn);
            }
            System.out.println();
            cmpt++;
        }
        if (avecNomCases) {
            afficheNomsCols();
        }
    }

    public static int rendIndexLgn(int lgn) {
        return 8 - lgn;
    }

    public static int rendIndexCol(char col) {
        return col - 'A';
    }

    private static void afficheNomsCols() {
        System.out.print("  ");
        for (char col = 'A'; col <= 'H'; col++) {
            System.out.print(" " + col + " ");
        }
        System.out.println();
    }

    public static void afficheEchiquierWaouh(char[][] echiquier, boolean avecNomCases) {
        final String FOND_NOIR = "\u001B[40m";
        final String FOND_GRIS = "\u001B[47m";
        final String RESET = "\u001B[0m";
        final String TXT_NOIR = "\u001B[30m";
        final String TXT_BLANC = "\u001B[37m";

        int cmpt = 1;
        if (avecNomCases) {
            afficheNomsCols();
        }
        for (int lgn = 8; lgn >= 1; lgn--) {
            if (avecNomCases) {
                System.out.print(lgn + " ");
            }
            for (char col = 'A'; col <= 'H'; col++) {
                int iLgn = rendIndexLgn(lgn);
                int iCol = rendIndexCol(col);
                if (cmpt % 2 == 0) {
                    System.out.print(FOND_NOIR);
                    System.out.print(TXT_BLANC);
                } else {
                    System.out.print(FOND_GRIS);
                    System.out.print(TXT_NOIR);
                }
                System.out.print(" ");
                System.out.print(echiquier[iLgn][iCol]);
                System.out.print(" ");
                cmpt++;
            }
            System.out.print(RESET);
            if (avecNomCases) {
                System.out.print(" ");
                System.out.print(lgn);
            }
            System.out.println();
            cmpt++;
        }
        if (avecNomCases) {
            afficheNomsCols();
        }
    }
}
